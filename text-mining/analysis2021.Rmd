...
data = read_excel("./data/GPAI AIPR_Living Repository.xlsx", sheet = "2021 Living Repository with Not")
data = data[-c(1,3,4), -c(1)]
...
project_data = data %>% 
  ...
  select(Label, Description, Category, Themes, Domain, Mission, name) 
...
project_rows = project_rows %>% 
  select(-Label) %>% 
  rename(Label = name) %>% 
  select(Label, Type, Description, Topic, Category, Themes, Domain, Mission)
